...
- Scripts
  ...
  - `bin/analyze-day` - Rebuilds `~/git/icuvisor` at its newest release, then runs
    ...
    landed, then sends it to Telegram as a document. Sources credentials from `.env` in
    the repo root, gitignored (override with `COACH_ENV`): `INTERVALS_ICU_ATHLETE_ID`
